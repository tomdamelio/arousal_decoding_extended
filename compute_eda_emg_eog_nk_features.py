...
def extract_EDA_measures(epochs):
    # Input -> EpochsEDA
    # Output -> n_epochs x 4 features (meanEDA and varEDA, phasic and tonic components)
    
    EDA_mean = epochs.get_data().mean(axis=2) # keep 2 EDA means
    EDA_var = epochs.get_data().var(axis=2) # keep 2 EDA vars
...
def run_subject(subject, cfg, condition):
    ...
    try:
        if feature_type == 'EDA':       
            ...
            picks_eda = mne.pick_channels(ch_names = epochs.ch_names ,include=['EDA_Phasic', 'EDA_Tonic'])       
            if int(subject) > 22:
