...
number_subject = subject_number[0]
...
#raw.drop_channels(["EDA_Phasic", "EDA_Tonic", "EDA_SMNA", "EMG_Amplitude_5", "EMG_Amplitude_6", "EOG_Cleaned"])
...
#raw = raw.apply_function(fun=lambda x: (10**9/x), picks=picks_EDA)
...
#eda_tonic = cvx[2]
#%%
# CORRER LAS TRES CELDAS SIGUIENTES PARA OBTENER PLOTS DE EDA PHASIC, TONIC Y SMNA
import matplotlib.pyplot as plt
fig, ax = plt.subplots(1, 1, figsize=[20, 8])
times = [i for i in range(len(raw_eda[:1700000]))]
ax.plot(times, raw_eda[:1700000], color='r', alpha = 0.5, label=f'True EDA')
...
#%%
raw_eda_smna = raw.copy().pick_channels(ch_names=['EDA_SMNA']).get_data()
raw_eda_smna = np.squeeze(raw_eda_smna)
fig, ax = plt.subplots(1, 1, figsize=[20, 8])
times = [i for i in range(len(raw_eda_smna[:1700000]))]
ax.plot(times, raw_eda_smna[:1700000], color='r', alpha = 0.5, label=f'True EDA')
ax.set_xlabel('Time (epochs)')
ax.set_ylabel(f'EDA Var')
ax.set_title(f'Plot test')
plt.legend()
